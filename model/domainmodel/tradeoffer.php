...
class TradeOffer extends Model
{
    ...
    public function __construct($tid, $dto = null, $new = false)
    {
        ...
        if ($new) {
            ...
            if (!$dto) {
                ...
                if (!is_object($dto)) {
                    throw new TradeException("Trade Offer id: {$tid} does not exist...");
                }
            }
    ...
    {
        if ($this->isNew()) {
            throw new InvalidActionException("Cannot revise a new trade that does not exist in database.");
        }
        $this->saveAll();
        $senderName = $this->getSenderName();
        return $this->sendTradeMessage(
            "Your Trade Offer from {$senderName} has been updated!",
    ...
    public function cancel()
    {
        ...
        $senderName = $this->getSenderName();
        return $this->sendTradeMessage(
            "Your trade request from {$senderName} is canceled.",
    ...
    public function accept()
    {
        ...
        return $this->sendTradeMessage(
            "Your trade request to {$recipientName} is sucessful.",
    ...
    public function decline()
    {
        ...
        $this->sendTradeMessage(
            "Your trade request to {$recipientName} was declined.",
    ...
    public function reverse($type = "")
    {
        $mysidia = Registry::get("mysidia");
        if ($this->isNew()) {
            throw new InvalidActionException("Cannot return a new trade that does not exist in database.");
        }
        $senderName = $this->getSenderName();
        if ($type) {
            $this->save("type", $type);
        ...
        return $this->sendTradeMessage(
            "Your Trade Offer from {$senderName} has been returned!",
