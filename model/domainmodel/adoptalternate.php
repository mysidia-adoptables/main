...
class AdoptAlternate extends Model{
    ...
    public function __construct($altInfo){	  
        ...
        if(!is_object($dto)) throw new AlternateNotfoundException("The alternate form {$altInfo} does not exist...");
	...
	public function getImage($fetchMode = ""){
	    if($fetchMode == Model::GUI) return new Image($this->image);
	    return $this->image;
	}

    public function getLevel($fetchMode = ""){
	    if($fetchMode == Model::MODEL) return new Level($this->adopt, $this->level);
    ...
    public function getLastAlt($fetchMode = ""){
	    if($fetchMode == Model::MODEL) return new static($this->lastalt);
        return $this->lastalt;
    }
