...
class RegistrationValidator extends Validator
{
    ...
    protected function checkUsername()
    {
        ...
        if (!$this->emptyValidate($username)) {
            throw new RegistrationException("username_empty");
        }
        if (in_array(strtolower((string) $username), ["admin", "system"])) {
            throw new RegistrationException("username_filter");
        }
        if (is_numeric($username[0]) || ($username != ltrim((string) $username))) {
            throw new RegistrationException("username_invalid");
        }
        $userExists = $this->datavalidate("users", ["username"], "username = :username", ["username" => $username]);
        if ($userExists) {
            throw new RegistrationException("username_exists");
        }
    }
    ...
    protected function checkPassword()
    {
        ...
        if (!$this->emptyValidate($password)) {
            throw new RegistrationException("password_empty");
        }
        if (strlen((string) $password) < 6) {
            throw new RegistrationException("password_length");
        }
    }
    ...
    protected function checkPassword2()
    {
        ...
        if (!$this->emptyValidate($password2)) {
            throw new RegistrationException("password_confirm");
        }
    ...
    protected function checkEmail()
    {
        ...
        if (!$this->emptyValidate($email)) {
            throw new RegistrationException("email_empty");
        }
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            throw new RegistrationException("email_invalid");
        }
    }
    ...
    protected function checkBirthday()
    {
        ...
        if (!$this->emptyValidate($birthday)) {
            throw new RegistrationException("birthday_empty");
        }
        try {
    ...
    {
        if (!$this->emptyValidate($this->form["avatar"])) {
            throw new RegistrationException("avatar");
        }
    }
    ...
    {
        $bio = $this->form["bio"];
        if ($bio && strlen((string) $bio) > 500) {
            throw new RegistrationException("bio");
        }
    }
    ...
    protected function checkColor()
    {
        ...
        if ($color && strlen((string) $color) > 20) {
            throw new RegistrationException("color");
        }
    }
    ...
    protected function checkNickname()
    {
        ...
        if ($nickname && strlen((string) $nickname) > 40) {
            throw new RegistrationException("nickname");
        }
    }
}
