...
{
    public function __construct(private readonly PoundAdoptable $adopt, private $cost, private readonly PoundSettings $settings, ArrayObject $validations)
    {
    ...
    protected function checkOwner()
    {
        ...
        if ($this->adopt->getOwner() || $this->adopt->getCurrentOwner()) {
            throw new PoundException("unlucky");
        }
    ...
    protected function checkNumber()
    {
        ...
        if ($this->settings->date == "yes") {
            $whereClause .= " AND dateadopt = '{$today->format('Y-M-D')}'";
        }
