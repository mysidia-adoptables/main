...
class TradeValidator extends Validator{
	...
	protected function checkRecipient(){
	    ...
	    if(!$recipientID) throw new TradeException("recipient_empty");
		else{
		    $sender = $this->offer->getSender(Model::MODEL);
            if($recipientID == $sender->getID()) throw new TradeInvalidException("recipient_duplicate");
            $recipient = $this->offer->getRecipient(Model::MODEL);
			if($recipient->getOption()->getTradeStatus() == 1){
				if(!$recipient->isFriend($sender)) throw new TradeException("recipient_privacy");
			}
		}
	}

    protected function checkPublic(){
	    if($this->offer->getRecipient()) throw new TradeException("recipient_public");
    }
    ...
    protected function checkAdoptOffered(){
	    if(!$this->offer->hasAdoptOffered()) return; 
        try{
    ...
    protected function checkAdoptWanted(){
	    if(!$this->offer->hasAdoptWanted()) return;  
        try{	
            $recipientID = $this->offer->getRecipient();
    ...
    protected function checkItemOffered(){
	    if(!$this->offer->hasItemOffered()) return; 
		...
		while($itemIterator->hasNext()){
			...
			if($item->isNew()) throw new TradeException("itemoffered"); 
            if(!$item->isTradable()) throw new TradeException("item");
		}
    }
    
    protected function checkItemWanted(){
	    if(!$this->offer->hasItemWanted()) return; 
        $itemWanted = $this->offer->getItemWanted();
		$itemIterator = $itemWanted->iterator();
		while($itemIterator->hasNext()){
			$iid = $itemIterator->next();
			$item = new OwnedItem($iid->getValue());
			if($item->isNew()) throw new TradeException("itemwanted");
            if(!$item->isTradable()) throw new TradeException("item");
		}
    }
    ...
    protected function checkCashOffered(){
	    if(!$this->offer->hasCashOffered()) return; 
        $cashOffered = $this->offer->getCashOffered();
		$cashLeft = $this->offer->getSender("model")->getMoney() - ($cashOffered + $this->settings->tax);
		if($cashLeft < 0) throw new TradeException("cashoffered");
    }
    
    protected function checkStatus(){
	    $status = $this->offer->getStatus();
		if($status != "pending" && $status != "moderate") throw new TradeException("status");
    }    
    
    protected function checkSpecies(){
	    if(empty($this->settings->species)) return;
		if($this->offer->hasAdoptOffered()) $this->checkMultipleSpecies($this->offer->getAdoptOffered());
		if($this->offer->hasAdoptWanted()) $this->checkMultipleSpecies($this->offer->getAdoptWanted());
    }
	...
	private function CheckMultipleSpecies(ArrayList $adopts){
		foreach($this->settings->species as $species){
		    ...
		    while($adoptIterator->hasNext()){
			    $aid = $adoptIterator->next();
		        $adopt = new OwnedAdoptable($aid->getValue());
			    if($adopt->getType() == $species) throw new TradeException("species");
		    }			
		}
	}
    ...
    protected function checkInterval(){
        ...
        if($mysidia->input->action() != "offer") return;	
	    $today = new Date;
        ...
        if($lastDate > $validDate) throw new TradeException("interval");      	
	}
    
    protected function checkNumber(){	  
        if($this->settings->number == 0) throw new TradeException("number");
		if($this->offer->hasAdoptOffered()) $this->checkNumbers($this->offer->getAdoptOffered());
        if($this->offer->hasAdoptWanted()) $this->checkNumbers($this->offer->getAdoptWanted());
		if($this->offer->hasItemOffered()) $this->checkNumbers($this->offer->getItemOffered());	
        if($this->offer->hasItemWanted()) $this->checkNumbers($this->offer->getItemWanted());				
    }
    
    private function checkNumbers(ArrayList $list){
	    if($this->settings->number < $list->size()) throw new TradeException("number");
	}
    
	protected function checkDuration(){
	    $today = new Date;
		$expirationDate = $this->offer->getDate()->modify("+{$this->settings->duration} day");	
		if($today > $expirationDate) throw new TradeException("duration");      
	}
    
    protected function checkUsergroup(){
		if($this->settings->usergroup == "all") return;
		...
		foreach($this->settings->usergroup as $usergroup){
		    if($senderGroup == $usergroup || $recipientGroup == $usergroup) return;   
		}
		throw new TradeException("usergroup");
    }
    
    protected function checkItem(){
	    if(!$this->settings->item) return;	
		if($this->offer->hasItemOffered()) $this->checkMultipleItem($this->offer->getItemOffered(), $this->offer->getSender());	
        if($this->offer->hasItemWanted()) $this->checkMultipleItem($this->offer->getItemWanted(), $this->offer->getRecipient());			
    }
	...
	private function checkMultipleItem(ArrayList $items, $owner){
		foreach($this->settings->item as $item){
		    ...
		    while($itemIterator->hasNext()){
			    ...
			    if($item->isNew() || !$item->isTradable()) throw new TradeException("item");
		    }				
		}	
	}    
}