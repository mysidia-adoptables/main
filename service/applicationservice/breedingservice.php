...
final class BreedingService extends MysObject{ 
    ...
    public function __construct(OwnedAdoptable $female, OwnedAdoptable $male, BreedingSettings $settings){
        $this->female = $female;
        $this->male = $male;
    ...
    public function getValidator(){
		...
		if(func_get_arg(0) == "all") $validations = new ArrayObject(["class", "gender", "owner", "species", "interval", "level", "capacity", "number", "cost", "usergroup", "item", "chance"]);
    ...
    public function getBabyAdopts($species = ""){
	    if($this->settings->method == "heuristic" || !$species) $this->heuristicBreed();
