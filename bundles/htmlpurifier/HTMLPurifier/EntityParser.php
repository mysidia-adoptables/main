...
class HTMLPurifier_EntityParser
{
    ...
    protected function entityCallback($matches)
    {
        ...
        if ($hex_part !== null && $hex_part !== "") {
            return HTMLPurifier_Encoder::unichr(hexdec((string) $hex_part));
    ...
    protected function nonSpecialEntityCallback($matches)
    {
        ...
        if ($is_num) {
            $is_hex = (@$entity[2] === 'x');
            $code = $is_hex ? hexdec((string) $matches[1]) : (int)$matches[2];
    ...
    protected function specialEntityCallback($matches)
    {
        ...
        if ($is_num) {
            $is_hex = (@$entity[2] === 'x');
            $int = $is_hex ? hexdec((string) $matches[1]) : (int)$matches[2];
