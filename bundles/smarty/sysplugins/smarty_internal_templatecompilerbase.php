...
abstract class Smarty_Internal_TemplateCompilerBase
{
    ...
    {
        $this->smarty = $smarty;
        $this->nocache_hash = str_replace(['.', ','], '_', uniqid(random_int(0, mt_getrandmax()), true));
    }
    ...
    private function compileTag2($tag, $args, $parameter)
    {
        ...
        foreach ($args as $arg) {
            if (!is_array($arg)) {
                ...
                foreach ($arg as $k => $v) {
                    if ($k == "'nocache'" && (trim((string) $v, "'\" ") == 'true')) {
                        $this->tag_nocache = true;
                    }
                }
            }
        ...
        if ((string) $text != '') {
            ...
            if ($this->parser->strip) {
                if (str_contains($text, '<')) {
                    ...
                    $_offset = 0;
                    if (preg_match_all('#@!@SMARTY:([0-9]+):SMARTY@!@#is', (string) $text, $matches,
    ...
    public function trigger_template_error($args = null, $line = null, $tagline = null)
    {
        ...
        if (in_array($this->template->source->type, ['eval', 'string'])) {
            $templateName = $this->template->source->type . ':' . trim((string) preg_replace('![\t\r\n]+!', ' ',
                                                                                    strlen($lex->data) > 40 ?
