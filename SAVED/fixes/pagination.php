...
class Pagination extends MysObject{
	...
	public function __construct($totalrows, $rowsperpage, $website, $page = 1){
	...
	public function showPage($margin = NULL, $padding = NULL){
		...
		if($this->getLastPage() > 1){
			if ($page > 1) $pagination .= "<li class='page-item'><a class='page-link' href='{$this->website}{$this->symbol}page-{$prev}'>Previous</a></li>";
			else $pagination .= "<li class='page-item disabled'><a class='page-link' href='#'>Previous</a></li>";
			
			
			if ($this->getLastPage() < 9){	
				for ($counter = 1; $counter <= $this->getLastPage(); $counter++){
					if ($counter == $page) $pagination .= "<li class='page-item active'><a class='page-link' href='#'>{$counter}</a></li>";
					else $pagination .= "<li class='page-item'><a class='page-link' href='{$this->website}{$this->symbol}page-{$counter}'>{$counter}</a></li>";					
				}
			}
