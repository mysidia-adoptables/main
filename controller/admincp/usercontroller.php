...
class UserController extends AppController
{
    ...
    public function edit($uid = null)
    {
        ...
        if ($mysidia->input->post("submit")) {
            ...
            if ($mysidia->input->post("email")) {
                $user->setEmail($mysidia->input->post("email"), Model::UPDATE);
            }
            if ($mysidia->input->post("pass1")) {
                ...
                if ($mysidia->input->post("emailpwchange") == "yes") {
                    $user->sendPasswordEmail($mysidia->input->post("pass1"));
                }
            }
            if (!$user->isRootAdmin() && is_numeric($mysidia->input->post("usergroup"))) {
                $mysidia->db->update("users", ["usergroup" => $mysidia->input->post("usergroup")], "uid = '{$user->getID()}'");
            }
    ...
    public function delete($uid = null)
    {
        if (!$uid) {
            ...
            if ($user->isCurrentUser()) {
                throw new InvalidActionException("current");
            }
            if ($user->isAdmin()) {
                throw new NoPermissionException("admin");
            }
            $user->delete();
        }
        $this->setField("user", $uid ? $user : null);
