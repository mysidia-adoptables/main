...
class VmessageController extends AppController
{
    public function __construct()
    {
        ...
        if ($mysidia->systems->vmessages != "enabled") {
    ...
    public function view($uid, $uid2)
    {
        $mysidia = Registry::get("mysidia");
        if (!$uid || !$uid2) {
            throw new InvalidIDException("user_nonexist");
        }
        if ($uid == $uid2) {
            throw new DuplicateIDException("user_same");
        }
        $user = new Member($uid);
    ...
    public function edit($vid)
    {
        ...
        if ($mysidia->input->post("submit")) {
            $vmessage->edit($mysidia->input->post("vmtext"));
        }
    }
