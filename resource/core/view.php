...
abstract class View extends MysObject
{
    ...
    {
        return $this->fields->get(new MysString($key));
    }
    ...
    /**
     * The addStyle method, add a user specified style to the header.
     * @param String $style
     * @param String $folder
     * @access public
     * @return void
     */
    protected function addStyle($style, $folder = "css/")
    {
        $frame = Registry::get("frame");
        $header = $frame->getHeader();
        if (!str_contains($style, "http")) {
            $path = Registry::get("path");
            $style = $path->getTempRoot() . $folder . $style;
        }
        $header->addStyle($style);
    }

    /**
     * The addScript method, add a user specified script to the header.
     * @param String $script
     * @param String $folder
     * @access public
     * @return void
     */
    protected function addScript($script, $folder = "js/")
    {
        $frame = Registry::get("frame");
        $header = $frame->getHeader();
        if (!str_contains($script, "http")) {
            $path = Registry::get("path");
            $script = $path->getTempRoot() . $folder . $script;
        }
        $header->addScript($script);
    }

    /**
     * The addTemplate method, add a customized template file to the template view.
     * There is no need to specify the .tpl file extension for the template file name.
     * @param String $template
     * @param String $folder
     * @access public
     * @return void
     */
    protected function addTemplate($template, $folder = "common/")
    {
        $this->template->assign("scriptPath", SCRIPTPATH);
        $this->template->assign("mysidia", Registry::get("mysidia"));
        if (!$this->fields->isEmpty()) {
            $iterator = $this->fields->iterator();
            while ($iterator->hasNext()) {
                $field = $iterator->next();
                $this->template->assign((string)$field->getKey(), $field->getValue());
            }
        }
        return $this->template->fetchTemplate($folder . $template);
    }
    ...
    {
        return "This is an instance of Mysidia View class.";
    }
}
